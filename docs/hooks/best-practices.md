...
| Hook Source                   | Trust Level  | Description                                                                                                      |
| :---------------------------- | :----------- | :--------------------------------------------------------------------------------------------------------------- |
| **System**                    | **High**     | Configured by system administrators (e.g., `/etc/gemini-cli/settings.json`, `/Library/...`). Assumed to be safe. |
| **User** (`~/.gemini/...`)    | **High**     | Configured by you. You are responsible for their safety.                                                         |
| **Extensions**                | **Variable** | You explicitly install these. Trust depends on the extension source (integrity).                                 |
| **Project** (`./.gemini/...`) | **Variable** | **Untrusted by default.** Safe in trusted internal repos; higher risk in third-party/public repos.               |
...
> actors from silently swapping a safe command for a malicious one.
